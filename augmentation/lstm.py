...
class AgeingEmulator:
    ...
    Methods
    -------
    fit(theta_sequences)              — train on list of per-cell θ histories
    predict(theta_seed)               — predict one step ahead
    rollout(theta_seed, n_steps)      — autoregressive multi-step prediction
    fine_tune(new_cell_theta,
              freeze_lstm)            — transfer learning for a new cell
    update(new_theta_sequence)        — alias for incremental fine_tune
    """
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        # Normalisation stats (fit on training data)
    ...
    def fit(self, theta_sequences: list[np.ndarray]) -> None:
        ...
        theta_sequences : list of 2-D arrays, each shape (n_cycles, 5)
            One array per training cell. Cycles must be in order.
        """
        # Compute normalisation from all training data
    ...
    def predict(self, theta_seed: np.ndarray) -> np.ndarray:
        ...
        Parameters
        ----------
        theta_seed : array shape (window, 5) or (≥window, 5)
            Recent θ history. Only the last `window` rows are used.

        Returns
        -------
        np.ndarray shape (5,) — predicted θ for next cycle
        """
    ...
    def rollout(self, theta_seed: np.ndarray, n_steps: int) -> np.ndarray:
        ...
        n_steps    : how many future cycles to predict
        ...
        # Sliding buffer: keeps last `window` steps
        buffer = seed[-self._window:].copy()   # (window, 5)

        with torch.no_grad():
            for _ in range(n_steps):
                buf_norm = self._normalise(buffer)
                x = torch.tensor(buf_norm, dtype=torch.float32) \
                        .unsqueeze(0).to(self._device)   # (1, window, 5)
    ...
    def fine_tune(
        ...
        """
        Transfer learning: adapt to a new (unseen) cell.

        Parameters
        ----------
        new_cell_theta : array shape (n_cycles, 5) — first observed cycles
        freeze_lstm    : if True, freeze all LSTM weights and only retrain
                         the output Linear projection layer
        epochs         : fine-tuning epochs (small, e.g. 20-50)
        lr             : learning rate (defaults to config value)
        """
        if self._net is None:
            raise RuntimeError("Call fit() before fine_tune().")

        if freeze_lstm:
            # Freeze everything except the output projection
        ...
        # Unfreeze all params after fine-tuning
        for param in self._net.parameters():
            param.requires_grad = True

    def update(self, new_theta_sequence: np.ndarray) -> None:
        """
        Incremental update — fine-tune on new observations from
        a cell already in service. Keeps LSTM weights trainable.
        """
    ...
    def _make_windows(
        ...
        """
        Convert a (n_cycles, 5) sequence into supervised windows.

        If the sequence is shorter than window+1, pad by repeating the
        first row so at least one training sample is produced.

        Returns
        -------
        X : (N, window, 5)   — input windows (normalised)
        y : (N, 5)           — next-step targets (normalised)
        """
    ...
    def _run_training(
        ...
        """MSE training loop with teacher forcing."""
        ...
        for epoch in range(n_epochs):
            ...
            if (epoch + 1) % max(1, n_epochs // 5) == 0:
                print(f"    [LSTM] epoch {epoch+1:4d}/{n_epochs}  "
...
"""
    ...
    # ---- Synthetic θ trajectories (3 training cells, 200 cycles each) ----
    # Simulate realistic ECM parameter drift over cycle life
    def make_theta_sequence(n_cycles: int, seed: int) -> np.ndarray:
        rng = np.random.default_rng(seed)
        k   = np.arange(n_cycles, dtype=float)

        R0  = 0.015 + 0.008  * (1 - np.exp(-k / 150)) \
              + rng.normal(0, 0.0003, n_cycles)
        R1  = 0.010 + 0.004  * (k / n_cycles) \
              + rng.normal(0, 0.0002, n_cycles)
        C1  = 3000  - 400    * (k / n_cycles) \
              + rng.normal(0, 5.0,   n_cycles)
        R2  = 0.008 + 0.003  * (k / n_cycles) \
              + rng.normal(0, 0.0001, n_cycles)
        C2  = 8000  - 800    * (k / n_cycles) \
              + rng.normal(0, 10.0,  n_cycles)

        return np.column_stack([R0, R1, C1, R2, C2])

    print("\nGenerating synthetic θ sequences for 3 training cells...")
    ...
    seed_seq   = train_seqs[0]          # use cell 0's history as seed
    ...
    # Check R0 is monotonically increasing (as expected for degradation)
    r0_increasing = np.sum(np.diff(pred_traj[:, 0]) > 0) / 49
    print(f"  R0 increasing in {r0_increasing*100:.0f}% of rollout steps "
    ...
    for n, v in zip(names, next_theta):
        print(f"  {n:3s} = {v:.5f}")

    # ---- Transfer learning: fine-tune on a new cell ----
    print("\nTransfer learning — freeze LSTM, retrain projection on new cell...")
    new_cell_seq = make_theta_sequence(50, seed=99)   # only 50 cycles observed
    emulator.fine_tune(new_cell_seq, freeze_lstm=True, epochs=20)

    rollout_new = emulator.rollout(new_cell_seq, n_steps=30)
    print(f"  Rollout after fine-tune — R0 range: "
    ...
    # Get prediction from current emulator BEFORE loading into a fresh one
    ...
    print(f"\nSave/load round-trip max diff: {diff:.2e}  (expect < 1e-4)")
    ...
    # Panel 1: R0 rollout vs true trajectory
    true_future = train_seqs[0][200 - 50:]   # last 50 of training cell
    ...
    axes[0].set_xlabel("Steps ahead"); axes[0].set_ylabel("R0 (Ω)")
    axes[0].set_title("Autoregressive rollout vs true"); axes[0].legend()

    # Panel 2: all 5 params normalised rollout
    ...
    axes[1].set_xlabel("Steps ahead"); axes[1].set_ylabel("Normalised θ")
    axes[1].set_title("All 5 params — 50-step rollout"); axes[1].legend()
